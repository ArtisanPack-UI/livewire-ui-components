...
<div>
    ...
    <x-artisanpack-header title="Hello" separator progress-indicator>
        ...
        <x-slot:actions>
            <x-artisanpack-button label="Filters" @click="$wire.drawer = true" responsive icon="o-funnel" />
        </x-slot:actions>
    </x-header>
    ...
    <x-artisanpack-card shadow>
        <x-artisanpack-table :headers="$headers" :rows="$users" :sort-by="$sortBy">
            @scope('actions', $user)
            <x-artisanpack-button icon="o-trash" wire:click="delete({{ $user['id'] }})" wire:confirm="Are you sure?" spinner class="btn-ghost btn-sm text-error" />
            @endscope
        </x-table>
    </x-card>
