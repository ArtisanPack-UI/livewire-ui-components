...
/**
 * Comprehensive unit tests for the Icon component.
 *
 ...
 */
class IconComponentTest extends ComponentTestCase
{
    protected string $componentClass = Icon::class;
    ...
    public function test_icon_string_properties(): void
    {
        $stringProperties = ['name', 'id', 'label', 'uuid'];
    ...
    public function test_icon_icon_method(): void
    {
        $component = $this->createComponent();

        if (method_exists($component, 'icon')) {
            try {
                $result = $component->icon();
    ...
    public function test_icon_label_classes_method(): void
    {
        $component = $this->createComponent();

        if (method_exists($component, 'labelClasses')) {
            try {
                $result = $component->labelClasses();
                // Add specific assertions based on expected return type
                $this->assertNotNull($result);
            } catch (Error $e) {
                if (str_contains($e->getMessage(), 'Call to a member function') &&
    ...
    public function test_icon_renders_successfully(): void
    {
        $component = $this->createComponent();
    ...
    public function test_icon_accessibility_compliance(): void
    {
        $component = $this->createComponent();
        $view      = $component->render();
        ...
        $this->assertTrue($validation['is_valid'],
            'Icon should have valid HTML structure. Issues: '.implode(', ', $validation['issues']),
        );
    }

    public function test_icon_security_against_xss(): void
    {
    ...
    public function test_icon_performance(): void
    {
        try {
            ...
            $this->assertLessThan(100, $performance['average_time'],
                'Icon rendering should be under 100ms on average',
            );
